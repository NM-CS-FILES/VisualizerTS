...
export class Canvas {
    ...
    drawRectangle(color, pos, width, height, fill = false) {
    ...
    convertPolarCoords(center, angle, radius) {
        return { x: center.x + Math.cos(angle) * radius, y: center.y + Math.sin(angle) * radius };
    }
    loadTexture(textureID, filePath) {
        sdl2bind.loadTextureBuffer(this._renderer, textureID, filePath);
    }
    drawTexture(textureID, pos) {
        setTexture(this._renderer, pos.x, pos.y, textureID);
    }
    static getScreenResolution() {
        return sdl2bind.getScreenRes();
    }
    getTextureResolution(textureID) {
        return sdl2bind.getTextureRes(this._renderer, textureID);
    }
    drawPath(path, pos = { x: 0, y: 0 }, color) {
